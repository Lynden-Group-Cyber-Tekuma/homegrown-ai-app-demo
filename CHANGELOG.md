...
## [2026-07-11]
### Removed
- LiteLLM proxy removed entirely; direct provider routing (model discovery) is now the only LLM path. Backend (`app/main.py`): the shared `litellm_client`, `LITELLM_BASE_URL`/`LITELLM_MASTER_KEY` config, `set_litellm_master_key()`, `_litellm_extra()`, the LiteLLM `/v1/models` fetch in `refresh_model_cache()` (the cache is now built solely from discovered provider models), the `litellm_key_enc` setting (lifespan load, `litellm_key_set` in `/app/settings`, and the `POST`/`DELETE /admin/litellm-key` endpoints). `_user_llm_client()`/`_guest_llm_client()` now always call the provider directly (per-user key, then shared key) and raise a clear error when no key is configured; Perplexity gained a direct-call base URL; the vestigial `local_openai` provider path (`_LOCAL_OPENAI_MODEL_IDS`) is gone. Admin UI: the LiteLLM Master Key row in Settings -> Security plus its checklist row, nav-dot logic, and save/clear JS. Chat UI: the "LiteLLM offline" picker note replaced, walkthrough LLM-call step now shows the direct provider call, unused "Local Models" picker group removed. Infrastructure: `litellm` service removed from `docker-compose.yml`, `litellm/config.yaml` and `postgres/init.sql` (which existed only to create the LiteLLM database) deleted. Docs: README LiteLLM proxy and Local OpenAI-compatible endpoint sections removed; CLAUDE.md architecture notes updated. The `litellm` Python package is retained as a library dependency for `token_counter.py` tokenization only - @duc.do
- Ollama (local models) support removed entirely. Backend (`app/main.py`): the `ollama` provider branch in `_detect_provider()`/`_model_meta()`, Ollama model auto-discovery in `refresh_model_cache()`, the direct-to-Ollama streaming bypass in `/chat/stream`, the `ollama_enabled`/`ollama_base_url`/`ollama_model_ids` application settings (load, parse, and PATCH handling), the `_update_litellm_ollama_models()` YAML writer, and all seven `/admin/ollama/*` endpoints (test, service status/start/stop, pull, cancel pull, delete model) together with the Docker SDK container-management helpers (`_get_docker_client`, `_find_ollama_container`). Admin UI (`admin.html`): the Settings → Ollama pane, service controls, model picker, pull progress UI, the Ollama Model Library browser modal and its catalogue, and all checklist/nav-dot/exit-gate logic tied to Ollama state. Chat UI (`index.html`): the "Local Models (Ollama)" picker group and default-model preference, the legacy Ollama toolbar badge, the "AI GW: Ollama" integration tab and code sample, and the Ollama row in the gateway provider table. Infrastructure: the `ollama` service, `ollama_data` volume, external network and `OLLAMA_BASE_URL` env removed from `docker-compose.yml`; the `ollama/*` wildcard entry removed from `litellm/config.yaml`; the Docker-socket mount, `COMPOSE_PROJECT_NAME` env, and entrypoint socket-group setup removed (they existed only for Ollama container control); `docker` and `pyyaml` dropped from dependencies; `certs/corporate-ca.pem` placeholder deleted; README Ollama section removed. Setup completeness now requires a provider API key (Ollama no longer counts as an LLM source) - @duc.do
